...
def portal_empresas_view(request):     
	...
	if request.method == "POST":
		...
		if (fi == "") & (ff == "") & (t_orden == ""):
			...
			lista=[]
			for p in ordenes:
				servicios = orden.objects.filter(id=p.id).servicios().values_list('id', flat=True)
				laboratorios = Laboratorio.objects.filter(servicio__id__in=servicios)

				resultados = p.resultados_laboratorio.all()
				...
				lista.append({'orden': p,'servicios':cadena, 'laboratorios': laboratorios})
			form = fechaRipsForm()	
			...
			usuario = usuario_empresa.objects.get(usuario=request.user)
			ordenes = orden.objects.filter(pk__icontains=t_orden, empresa=usuario.empresa).order_by('fecha')
			lista=[]
			for p in ordenes:
				servicios = orden.objects.filter(id=p.id).servicios().values_list('id', flat=True)
				laboratorios = Laboratorio.objects.filter(servicio__id__in=servicios)
		...
		if form.is_valid():
			if(miembroUsuario.check_password(form.cleaned_data['contrasenaAnterior']) and \
			   form.cleaned_data['contrasenaNueva'] == form.cleaned_data['contrasenaNuevaVerificacion']):
				...
				miembroUsuario.save()
				if miembroUsuario.has_perm('home.es_coordinador_general'):
					return HttpResponseRedirect('/portal_empresas/')
				return HttpResponseRedirect("/")
			else:
